...
class TruthFinder(object):
    ...
    def confidence_score(self, df):
        ...
        # Adding row numbers
        df = df.with_row_index("row_number")
        # Calculate confidence for each fact
        for row in df.iter_rows(named=True):
            # Get the trustworthiness score of each source providing f
            ts = df.filter(pl.col("fact") == row["fact"]).select("trustworthiness").to_series().to_list()
            ...
            df = df.with_columns(
                pl.when(pl.arange(0, df.height) == row["row_number"])
                .then(confidence_score)
                .otherwise(pl.col("fact_confidence"))
                .alias("fact_confidence")
            )
        # Return the data
        return df
    ...
    def adjusted_confidence_score(self, df):
        ...
        for row1 in df.iter_rows(named=True):
            # Get the first fact
            f1 = row1["fact"]
            # Initialize the sum
            sum = 0
            # Iterate through the facts
            for row2 in df.unique(subset=["fact"]).iter_rows(named=True):
                # Get the second fact
                f2 = row2["fact"]
                # Compare the facts
                if f1 == f2:
            ...
            adjusted[row1["row_number"]] = self.influence_related * \
                sum + row1["fact_confidence"]
        # Iterate through the facts
        for row1 in df.iter_rows(named=True):
            # Update the values of confidence scores
            df = df.with_columns(
                pl.when(pl.arange(0, df.height) == row1["row_number"])
                .then(adjusted[row1["row_number"]])
                .otherwise(pl.col("fact_confidence"))
                .alias("fact_confidence")
            )
        # Return the data
    ...
    def update_fact_confidence(self, df):
        ...
        for object_ in df["object"].unique():
            ...
            # Calculate and update the fact confidences
            fc = self.compute_fact_confidence(d)["fact_confidence"]
            # Initialize counter
            counter = 0
            for index, value in enumerate(indices):
                if value:
                    df = df.with_columns(
                        pl.when(pl.arange(0, df.height) == pl.lit(index))
                        .then(fc[counter])
                        .otherwise(pl.col("fact_confidence"))
                        .alias("fact_confidence")
                    )
    ...
    def update_source_trustworthiness(self, df):
        ...
        for source in df["source"].unique():
            ...
            cs = df.filter(indices)["fact_confidence"]
            # Initialize counter
            counter = 0
            ...
            for index, value in enumerate(indices):
                if value:
                    df = df.with_columns(
                        pl.when(pl.arange(0, df.height) == pl.lit(index))
                        .then(sum(cs) / len(cs))
                        .otherwise(pl.col("trustworthiness"))
    ...
    def run(self, dataframe, max_iterations=200, threshold=1e-6):
        ...
        dataframe = dataframe.with_columns(
            pl.Series(name="fact_confidence", values=np.zeros(dataframe.height))
        )
        # Loop until max_iterations
        for i in range(max_iterations):
            # Remove duplicates for sources and get trustworthiness for each
            t1 = dataframe.unique(subset=["source"]).select("trustworthiness").to_series().to_list()
            # Perform an iteration
            dataframe = self.iteration(dataframe)
            # Remove duplicates for sources and get trustworthiness for each
            t2 = dataframe.unique(subset=["source"]).select("trustworthiness").to_series().to_list()
            try:
